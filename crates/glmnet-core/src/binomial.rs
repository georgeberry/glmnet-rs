...
/// `b`, `q`, `v`, `r`, and the strong set carry between lambdas.
// Specialized to `Dense` rather than generic over `DesignMatrix`: the GLM
// solvers need weighted per-column operations (`sum_i v_i x_ij^2`, the working
// residual update `r -= diff * v .* x_k`, the linear-predictor accumulation)
// that do not reduce to the trait's `dot`/`axpy`. When the sparse backend lands
// (see docs/PORTING.md), the right trait surface for those weighted ops will be
// clear; guessing it now would likely be wrong.
struct Point<'a> {
    x: &'a Dense,
    y: &'a [f64],
...
impl<'a> Point<'a> {
    ...
    fn new(
        ...
        // Seed |grad_j| = |<x_j, r>| for the strong-rule screen at m == 1.
        for j in 0..p {
            if pt.ju[j] {
    ...
    /// Weighted column variance `sum_i v_i x_ij^2`. Recomputed each IRLS step.
    #[inline]
    fn compute_xv(&self, k: usize) -> f64 {
        let col = self.x.col(k);
        crate::matrix::wdot4(&self.v, col, col)
    }
    ...
    fn update_one(&mut self, k: usize, full: bool, l1: f64, l2: f64) -> Result<(), PointErr> {
        let gk = self.x.dot(k, &self.r); // <x_k, r>, r = w*(y-q)
        let a_old = self.b[k];
        ...
        // r -= diff * (v .* x_k)
        for ((ri, vi), xi) in self.r.iter_mut().zip(&self.v).zip(self.x.col(k)) {
            *ri -= diff * vi * xi;
        }
        Ok(())
    }
    ...
    fn update_intercept(&mut self) {
        ...
        if d != 0.0 {
            ...
            for (ri, vi) in self.r.iter_mut().zip(&self.v) {
                *ri -= d * vi;
            }
        }
    }
    ...
    fn update_irls_invariants(&mut self) -> bool {
        let n = self.x.nrows();
        // Linear predictor eta = b0 + sum_{k active} b_k x_.k, accumulated
        // column-by-column (offset is zero). The per-i additions still happen in
        // active-set order, so this is numerically identical to the row-wise
        // form -- but the inner loop is now sequential in one column, so it is
        // cache-friendly and vectorizes.
        // Swap the scratch buffer out so we can write it while reading self.ia /
        // self.b / self.x; swapped back before returning (no allocation).
...
pub fn lognet(
    ...
    for m in 0..nlam {
        ...
        // Mirror glmnetpp control flow exactly: below the `mnl` guard (or with a
        // user lambda grid) the path always continues; otherwise the dfmax/fdev/
        // devmax break is tested; only if none fire is the working-variance
        // collapse (`!cont`) allowed to stop the path.
